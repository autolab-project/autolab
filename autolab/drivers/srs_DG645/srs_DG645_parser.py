...
usage:    autolab-drivers [options] arg 
    ...
    Note: Arbitrary waveform available only using a python terminal
        ...
        parser.add_argument("-c", "--channels", type=str, dest="channels", default=None, help="Set the channels to act on/acquire from. channels will be a list of outputs('AB','CD','EF','GH') or independant channels ('A','B','C','D','E','F','G','H'). WARNING: you can only change the relative delay between independant channels, you must act on outputs for other options. Pre-configured options only allow you to act on either outputs or channels[single('A') or couple('A','B') only] with a single command => use several commands or -m option instead." )
